...
app.get('/player_info/select_player',function(req,res){
  ...
  var allInfo = 'select * from football_players where id=' + player_id + ';';
  var gamesPlayed = 'select count(*) from football_games inner join football_players on football_players.id=any(football_games.players) and football_players.id = ' + player_id + ';';
  db.task('get-everything', task =>{
    return task.batch([
      task.any(nameId),
      task.any(allInfo),
